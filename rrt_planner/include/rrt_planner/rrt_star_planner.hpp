...
public:
  RRT(const ros::NodeHandle& f_node_handle, int max_count, int delta_max, Point2d start, bool goalReached, Point2d goal,
      int goal_tolerance, int epsilon);
  ...
  int computeTotalCost(int id);

  int computeCost(Point2d& pt1);

  void optimiseNode(Point2d& pt1);

  void rewireNodes(std::vector<Point2d> vicintiy_points);
